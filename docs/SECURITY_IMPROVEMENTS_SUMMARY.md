...
DB_PASSWORD="password"  # ハードコーディング
```
...
## 🚨 緊急: Git履歴に機密情報が含まれています

### ⚠️ 重大な問題

このリポジトリの**過去のコミット履歴**には、以下の機密情報が含まれています：

- データベースの認証情報（ユーザー名、パスワード、ホスト名）
- アプリケーションの操作パスワード
- サーバー接続情報

`.gitignore`に追加しても、**過去のコミット履歴からは削除されません**。

### 📋 必須対応（引き渡し前に実施）

1. **すべてのパスワードを即座に変更してください**
   - データベースパスワード
   - アプリケーション操作パスワード
   - SSH/サーバーパスワード（念のため）

2. **新しいデータベースを作成してください**
   - 現在は個人用のデータベースを使用
   - SE部用の新しいデータベースを作成
   - スキーマのみコピー（データは必要に応じて移行）

3. **リポジトリの取り扱いを決定してください**

   以下のいずれかを選択：

   **選択肢A: 現在のリポジトリを使い続ける**
   - リポジトリをプライベート設定にする
   - すべてのパスワードが変更済みであることを確認
   - 履歴に機密情報が含まれることを承知の上で使用

   **選択肢B: 新しいリポジトリを作成する（推奨）**
   - SE部のGitHub組織で新規リポジトリを作成
   - 現在のコード（最新版のみ）をコピー
   - 履歴なしでクリーンスタート
   - より安全
